...
- (id)initWithFrame:(CGRect)frame andStars:(NSUInteger)_numberOfStars atHeight:(CGFloat)_atHeight{
	self = [super initWithFrame:frame];
	if (self) {
...
- (void)layoutSubviews {
	for (int i=0; i < numberOfStars; i++) {
		[(DLStarView*)[self subViewWithTag:i] centerIn:self.frame with:numberOfStars atHeight:atHeight ];
	}
}
...
- (void)disableStarsDownToExclusive:(NSUInteger)idx {
...
- (void)disableStarsDownTo:(NSUInteger)idx {
	for (int i=numberOfStars; i >= idx; --i) {
		UIButton *b = (UIButton*)[self subViewWithTag:i];
		b.highlighted = NO;
	}
...
- (void)enableStarsUpTo:(NSUInteger)idx {
	for (int i=0; i <= idx; i++) {
		UIButton *b = (UIButton*)[self subViewWithTag:i];
		b.highlighted = YES;
	}
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
	...
	if (pressedButton) {
		NSUInteger idx = pressedButton.tag;

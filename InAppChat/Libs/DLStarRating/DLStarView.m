...
- (id)initWithDefault:(UIImage*)star highlighted:(UIImage*)highlightedStar position:(int)index allowFractions:(BOOL)fractions {
	...
	if (self) {
        ...
        self.frame = CGRectMake((star.size.width*index), 0, star.size.width+20, star.size.height+kEdgeInsetBottom+10);
...
- (void)centerIn:(CGRect)_frame with:(int)numberOfStars atHeight:(CGFloat)atHeight{
...
//	float newY = (frameHeight-height)/2;
    ...
    self.frame = CGRectMake((size.width*self.tag)+10 , newY, size.width, size.height);
//    self.frame = CGRectMake(gapToApply, newY, size.width, size.height);
}
